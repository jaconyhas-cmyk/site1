...
export class VideoService {
  ...
  private static readonly CACHE_KEYS = {
    ALL_VIDEOS: 'videos_all',
    VIDEO_PREFIX: 'video_',
    THUMBNAILS: 'thumbnails_cache',
  };

  // Cache TTL configurations
  private static readonly CACHE_TTL = {
    VIDEOS: 10 * 60 * 1000, // 10 minutes for video list
    SINGLE_VIDEO: 30 * 60 * 1000, // 30 minutes for individual videos
    THUMBNAILS: 60 * 60 * 1000, // 1 hour for thumbnails
  };

  // Método público para limpar o cache
  ...
  // Get all videos with sorting options and optimized caching
  static async getAllVideos(sortOption: SortOption = SortOption.NEWEST, searchQuery: string = ''): Promise<Video[]> {
    try {
      // Para pesquisas, não usar cache
      if (searchQuery && searchQuery.trim() !== '') {
        console.log('Searching videos without cache');
        return await this.searchVideos(searchQuery, sortOption);
      }

      // Usar cache otimizado para listagem completa
      const cacheKey = `${this.CACHE_KEYS.ALL_VIDEOS}_${sortOption}`;
      
      return await cacheService.getOrSet(
        cacheKey,
        async () => {
          console.log('Fetching and processing all videos...');
          
          // Buscar vídeos do database
      const videoDataList = await jsonDatabaseService.getAllVideos();
      
      // Converter para formato do frontend
      let videos = videoDataList.map(videoData => this.convertVideoData(videoData));
      
          // Processar thumbnails em paralelo para melhor performance
          await this.processThumbnailsInParallel(videos);
          
          // Aplicar ordenação
          videos = this.sortVideos(videos, sortOption);
          
          console.log(`Processed ${videos.length} videos with sorting: ${sortOption}`);
          return videos;
        },
        { 
          ttl: this.CACHE_TTL.VIDEOS,
  ...
  // Método otimizado para pesquisa
  private static async searchVideos(searchQuery: string, sortOption: SortOption): Promise<Video[]> {
    const videoDataList = await jsonDatabaseService.getAllVideos();
    
    // Converter e filtrar
        const trimmedQuery = searchQuery.trim().toLowerCase();
    let videos = videoDataList
      .map(videoData => this.convertVideoData(videoData))
      .filter(video => 
          video.title.toLowerCase().includes(trimmedQuery) || 
          video.description.toLowerCase().includes(trimmedQuery)
        );

    // Processar thumbnails apenas para resultados da pesquisa
    await this.processThumbnailsInParallel(videos);
  ...
  // Processar thumbnails em paralelo para melhor performance
  private static async processThumbnailsInParallel(videos: Video[]): Promise<void> {
    const thumbnailPromises = videos.map(async (video) => {
        const thumbnailId = video.thumbnailFileId || video.thumbnail_id;
        
        if (thumbnailId) {
        // Verificar cache de thumbnail primeiro
        const thumbnailCacheKey = `${this.CACHE_KEYS.THUMBNAILS}_${thumbnailId}`;
        
        try {
          video.thumbnailUrl = await cacheService.getOrSet(
            thumbnailCacheKey,
            () => wasabiService.getThumbnailUrl(thumbnailId),
            { 
              ttl: this.CACHE_TTL.THUMBNAILS,
              storage: 'localStorage'
            }
          );
          } catch (error) {
          console.error(`Error loading thumbnail for video ${video.$id}:`, error);
          video.thumbnailUrl = this.getPlaceholderThumbnail();
        }
      } else {
        video.thumbnailUrl = this.getPlaceholderThumbnail();
      }
