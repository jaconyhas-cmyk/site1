...
const LoadingModal: React.FC<LoadingModalProps> = ({ open, onClose }) => {
  ...
  useEffect(() => {
    ...
    // Aguardar 3 segundos para melhor UX
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose} // Allow closing after 2 seconds
      disableEscapeKeyDown={timeLeft > 1}
      maxWidth="sm"
      fullWidth
      PaperProps={{
        sx: {
    ...
    >
      <DialogContent sx={{ p: 4, textAlign: 'center', position: 'relative' }}>
        ...
        <Box sx={{ position: 'relative', zIndex: 1 }}>
          ...
          >
            Please wait {timeLeft} second{timeLeft !== 1 ? 's' : ''}
          </Typography>
